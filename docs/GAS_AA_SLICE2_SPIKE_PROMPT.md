...
> **● V2 PASSED 2026-06-26 (runtime-proven).** (a) owner EOA pre-deploy = `currentUser.evmAccounts[0]` (cdp-core `User.evmAccounts`), read at `setup/page.tsx:530` before deploy. (b) browser signer = `useSignEvmHash({ evmAccount: EvmAddress, hash: Hex }) → { signature }` (cdp-hooks 0.0.76). The runtime **sign → ECDSA-recover → owner** proof RAN on a minted `createEndUser` subject (recovered signer == owner EOA `0x91e6…c46b`) — see [GAS_AA_SLICE2_STEP0_FINDINGS.md](GAS_AA_SLICE2_STEP0_FINDINGS.md) §"V2(b) browser sign-proof — PASS". The prepare→browser-sign→submit architecture is **unblocked**; `useSignEvmHash` is the confirmed primitive (backend can't sign end-user EOAs — non-custodial). The earlier "inspection only / not yet wired" caveat is closed.

## V3 — sponsored setup creates a REGISTERABLE permission (staging, the crux)

**V3 step 0 — pre-deploy address convergence (HARD GATE) + the banked initCode.** Run this _before_ building anything; it decides whether we can deploy to the right address at all.
