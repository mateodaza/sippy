...
| --------------------------------------- | ------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `send`, `receive`                       | **score projector** off `onchain.transfer` (same hook as `onchain_writer`)                                         | derived from the source of truth; idempotent on the same `id`                                                                                                                                      |
...
| `new_counterparty`                      | **score projector** — first ≥$1 verified send to a never-paid cp                                                   | one per `(wallet, cp)`; flagged state reconciled (see below)                                                                                                                                       |
| `first_send`, `onramp_used`             | **recompute** (`#season/recompute.reconcileDerivedEvents`) — NOT inline                                            | rebuilt every score rebuild from the wallet's currently-UNFLAGGED value-outs (`onramp_used` via `rebuildOnrampRealization`: delete-all + FIFO replay), so a sybil flag/clear can't double-allocate |
| `offramp`                               | `#season/emissions` (off-ramp completion hook)                                                                     | high weight; KYC-gated; its on-ramp realization is rebuilt by the recompute it triggers                                                                                                            |
| `referral_unlock` / `referral_retained` | `#season/referral` (projector unlock detection + season job)                                                       | see §6                                                                                                                                                                                             |
| `active_week`                           | **season job** (`emitActiveWeeks`: add qualifying + delete-stale)                                                  | ≥1 unflagged qualifying value-out that week; job-cadenced (not in a plain recompute)                                                                                                               |

The projector is a second, idempotent consumer of the transfer feed (it never touches `onchain_writer`). **Realization is rebuild-owned, not inline:** `reconcileDerivedEvents` runs at the top of every score rebuild and re-derives `first_send`, `new_counterparty`, and the whole on-ramp realization (`onramp_used` + `pending_remaining`) from the wallet's current unflagged value-outs. That is what makes a flagged send earn nothing _anywhere_ (not just on the raw send), and what makes sybil-flag → clear safe against double-allocating a pending on-ramp. `active_week` is the one derived verb left to the season job.
