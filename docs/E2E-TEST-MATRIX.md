...
### 8.3 Send via EOA Wallet (WhatsApp Wallet)
- [ ] Dual-wallet UI: both WhatsApp EOA wallet and smart account shown as selectable cards
- [ ] Auto-selects funded wallet by default
- [ ] Send from EOA wallet via POST /api/send (SpendPermission)
- [ ] GasRefuel auto-funds gas before transfer
- [ ] Transaction receipt shows correct wallet source
