...
export default async function HomePage() {
  ...
  return (
    <div className='min-h-screen bg-[var(--bg-primary)] font-sans text-[var(--text-primary)] antialiased'>
      ...
      <section className='relative h-screen max-h-[900px] pt-16 sm:pt-20 pb-8 sm:pb-12 overflow-hidden flex flex-col justify-center items-center bg-[var(--bg-primary)] px-2 sm:px-6 lg:px-8 registration-marks'>
        ...
        <div className='relative z-10 w-full max-w-[75vw] sm:max-w-none sm:px-14 lg:px-16 mx-auto flex flex-col items-center text-center'>
          ...
          <div className='mt-4 sm:mt-8 flex justify-center gap-2 sm:gap-8 flex-wrap'>
            {(['landing.tags.1', 'landing.tags.2', 'landing.tags.3', 'landing.tags.4'] as const).map(
              (key, i) => (
      ...
      {/* ── Marquee Band ── */}
      <div className='bg-brand-primary dark:bg-[#0a0a0a] text-white py-3 sm:py-6 border-y border-white/30 dark:border-brand-primary/25 dark:shadow-[0_0_40px_rgba(0,175,215,0.10),inset_0_1px_0_rgba(0,175,215,0.15),inset_0_-1px_0_rgba(0,175,215,0.15)] relative z-20' aria-hidden='true'>
        <Marquee className='[--duration:40s] [--gap:2rem] sm:[--gap:4rem]'>
          {(['landing.marquee.1', 'landing.marquee.2', 'landing.marquee.3', 'landing.marquee.4'] as const).map((key, i) => (
            <span
              key={i}
        ...
        <div className='max-w-[75vw] sm:max-w-7xl mx-auto sm:px-6 lg:px-8'>
          <ScrollReveal>
            <div className='text-center mb-8 sm:mb-16'>
              <span className='spec-label block mb-3 sm:mb-4'>{t('landing.process.label', lang)}</span>
          ...
          <div className='grid grid-cols-1 lg:grid-cols-[1fr_auto_1fr] gap-8 lg:gap-16 items-center max-w-5xl mx-auto'>
            ...
            <ScrollReveal direction='left' className='space-y-12 text-right hidden lg:block max-w-xs ml-auto' aria-hidden='true'>
              <div>
                ...
                <p className='text-[var(--text-secondary)] leading-relaxed text-base'>
                  {t('landing.step1.desc.desktop', lang).split('\n').map((line, i) => (
              ...
              <div>
                ...
                <p className='text-[var(--text-secondary)] leading-relaxed text-base'>
                  {t('landing.step2.desc.desktop', lang).split('\n').map((line, i) => (
                    <span key={i}>{i > 0 && <br />}{line}</span>
                  ))}
                </p>
              </div>
            ...
            <ScrollReveal direction='right' delay={0.3} className='space-y-12 hidden lg:block max-w-xs' aria-hidden='true'>
              <div>
                ...
                <p className='text-[var(--text-secondary)] leading-relaxed text-base'>
                  {t('landing.step3.desc.desktop', lang).split('\n').map((line, i) => (
                    <span key={i}>{i > 0 && <br />}{line}</span>
                  ))}
                </p>
              </div>
              <div className='pt-4'>
                <p className='text-[var(--text-secondary)] text-sm leading-relaxed'>
                  {t('landing.step3.note', lang).split('\n').map((line, i) => (
                    <span key={i}>{i > 0 && <br />}{line}</span>
                  ))}
                </p>
              </div>
            ...
            <div className='lg:hidden space-y-5 sm:space-y-8'>
              {([
                { stepKey: 'landing.step1.num', titleKey: 'landing.step1.title', descKey: 'landing.step1.desc' },
                { stepKey: 'landing.step2.num', titleKey: 'landing.step2.title', descKey: 'landing.step2.desc' },
                { stepKey: 'landing.step3.num', titleKey: 'landing.step3.title', descKey: 'landing.step3.desc' },
              ] as const).map((s) => (
                <div key={s.stepKey} className='panel-frame rounded-xl sm:rounded-2xl p-4 sm:p-6'>
                  ...
                  <p className='text-[var(--text-secondary)] leading-relaxed text-base'>{t(s.descKey, lang)}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className='grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-20 items-center'>
            ...
            <ScrollReveal direction='left'>
              ...
              <div className='grid grid-cols-2 sm:flex sm:flex-wrap gap-2.5 sm:gap-5'>
                {(['landing.pill.1', 'landing.pill.2', 'landing.pill.3', 'landing.pill.4', 'landing.pill.5', 'landing.pill.6'] as const).map((key) => (
                  <div
                    key={key}
          ...
          <div className='py-10 sm:py-14 grid grid-cols-1 md:grid-cols-[1fr_auto] gap-8 items-start'>
            ...
            <div>
              ...
              <p className='font-mono text-sm text-[var(--text-secondary)] tracking-[0.12em] uppercase leading-relaxed max-w-xs'>
                {t('landing.footer.desc', lang).split('\n').map((line, i) => (
                  <span key={i}>{i > 0 && <br />}{line}</span>
                ))}
              </p>
            </div>
