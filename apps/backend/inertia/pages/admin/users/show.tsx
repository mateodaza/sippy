    ...
    return (
      <AdminLayout>
        ...
        <div className="flex flex-col items-center justify-center py-20">
          ...
          <Link
            href="/admin/users"
            className="mt-3 text-sm font-medium text-sippy hover:text-sippy-dark"
          >
            Back to users
          </Link>
        </div>
          ...
          {details.map((d) => (
            <div key={d.label}>
              ...
              {d.badge ? (
                ...
                >
                  <span
                    className={`mr-1.5 h-1.5 w-1.5 rounded-full ${d.badgeActive ? 'bg-sippy' : 'bg-gray-400'}`}
                  />
                  {d.value}
                </span>
              ) : (
                <div
                  className={`text-sm text-slate-700 ${d.mono ? 'break-all font-mono text-xs' : ''}`}
                >
                  {d.value}
                </div>
              )}
