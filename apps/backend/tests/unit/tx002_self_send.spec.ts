...
import { formatSelfSendMessage, formatConcurrentSendMessage } from '#utils/messages'
...
test.group('Group A | Self-send block', () => {
  test('A-01: send to own phone (immediate path, amount ≤ threshold) → self-send message sent; sendHandler NOT called; pendingTxs empty', async ({
    assert,
  }) => {
  ...
  test('A-02: send to different phone (immediate path) → no self-send message; sendHandler called', async ({
    assert,
  }) => {
  ...
  test('A-03: send to own phone (above threshold / confirmation path) → self-send message sent; pending tx NOT stored', async ({
    assert,
  }) => {
...
test.group('Group B | Concurrent send protection (immediate path)', () => {
  test('B-01: activeSendsSet pre-seeded with user phone → concurrent message sent; sendHandler NOT called', async ({
    assert,
  }) => {
  ...
  test('B-02: activeSendsSet empty → send executes normally; set empty after completion', async ({
    assert,
  }) => {
  ...
  test('B-03: sendHandler throws → activeSendsSet still cleared after (finally fires on throw)', async ({
    assert,
  }) => {
    ...
    const activeSendsSet = new Set<string>()
    const fakeMsg = async (..._args: any[]) => {}
    const throwingHandler = async (..._args: any[]) => {
...
test.group('Group C | Confirm path concurrent protection', () => {
  test('C-01: confirm with pending + activeSendsSet pre-seeded → concurrent message sent; sendHandler NOT called; pending tx REMAINS', async ({
    assert,
  }) => {
  ...
  test('C-02: confirm with pending + empty activeSendsSet → sendHandler called; set empty after; pending consumed', async ({
    assert,
  }) => {
...
test.group('Group D | Self-send before threshold check', () => {
  test('D-01: send to self above threshold → self-send fires first; no pending stored', async ({
    assert,
  }) => {
...
test.group('Group T | Safety valve timer', () => {
  test('T-01: safety valve clears activeSendsSet after timeout when processing hangs', async ({
    assert,
  }) => {
    const activeSendsSet = new Set<string>()
