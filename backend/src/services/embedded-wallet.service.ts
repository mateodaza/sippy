...
function getCdpClient(): CdpClient {
  if (!cdpClient) {
    ...
    console.log('✅ CDP Client initialized for embedded wallets');
  }
  return cdpClient;
}
...
export async function getSippySpenderAccount() {
  if (!sippySpenderAccount) {
    ...
    console.log(`✅ Sippy spender wallet: ${smartAccount.address}`);
  }
...
export async function getEmbeddedWallet(
  phoneNumber: string
): Promise<EmbeddedUserWallet | null> {
  try {
    ...
    console.error(`❌ Failed to get embedded wallet for +${phoneNumber}:`, error);
    throw error;
  }
}
...
export async function sendWithSpendPermission(
  ...
  if (!userWallet.spendPermissionHash) {
    throw new Error(
      'No spend permission found. Please set up your wallet at sippy.lat/setup'
    );
  }
  ...
  try {
    ...
    if (!permissionEntry) {
      ...
      console.log(`⚠️ Stored permission hash not found, using most recent permission`);
    }

    console.log(`✅ Found spend permission: ${permissionEntry.permissionHash}`);
    ...
    if (allowanceInfo) {
      ...
      console.log(`✅ Allowance check passed: $${amount} <= $${allowanceInfo.remaining.toFixed(2)} remaining`);
    }
    ...
    console.log(`📝 Executing batched spend + transfer in one transaction...`);
    ...
    console.log(`✅ Transfer complete! Hash: ${userOp.transactionHash}`);

    // Update last activity
    await query(
      'UPDATE phone_registry SET last_activity = $1 WHERE phone_number = $2',
      [Date.now(), fromPhoneNumber]
    );
...
export async function sendToPhoneNumber(
  ...
  if (toEmbeddedWallet) {
    return await sendWithSpendPermission(
      fromPhoneNumber,
      toEmbeddedWallet.walletAddress,
      amount
    );
  }
  ...
  const { getUserWallet } = await import('./cdp-wallet.service.js');
...
export async function getRemainingAllowance(phoneNumber: string): Promise<{
  ...
  try {
    ...
    const spentAmount = parseFloat(
      ethers.utils.formatUnits(periodSpend, USDC_DECIMALS)
    );
...
export async function initSpenderWallet(): Promise<void> {
  try {
    const spenderAccount = await getSippySpenderAccount();
    console.log(`\n🏦 Sippy Spender Wallet initialized:`);
    console.log(`   Address: ${spenderAccount.address}`);
    console.log(`   Network: ${NETWORK}`);

    // Validate environment
    if (!SIPPY_SPENDER_ADDRESS) {
      console.warn(
        '⚠️  SIPPY_SPENDER_ADDRESS not set in env. Using dynamically created wallet.'
      );
      console.log(`   Set SIPPY_SPENDER_ADDRESS=${spenderAccount.address} in .env`);
    } else if (
      SIPPY_SPENDER_ADDRESS.toLowerCase() !== spenderAccount.address.toLowerCase()
    ) {
      console.warn(
        '⚠️  SIPPY_SPENDER_ADDRESS in env does not match created wallet!'
      );
      console.log(`   Env: ${SIPPY_SPENDER_ADDRESS}`);
      console.log(`   Created: ${spenderAccount.address}`);
    }
  } catch (error) {
    console.error('❌ Failed to initialize spender wallet:', error);
    throw error;
  }
}
