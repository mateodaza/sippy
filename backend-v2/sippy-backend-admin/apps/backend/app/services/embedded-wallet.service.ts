...
import { TransferResult } from '#types/index';
...
export async function sendWithSpendPermission(
  ...
  try {
    ...
    if (allowanceInfo) {
      ...
      logger.info(`Allowance check passed: $${amount} <= $${allowanceInfo.remaining.toFixed(2)} remaining`);
    }
...
export async function sendToPhoneNumber(
  ...
  const { getUserWallet } = await import('#services/cdp-wallet.service');
...
export async function initSpenderWallet(): Promise<void> {
  try {
    ...
    if (!SIPPY_SPENDER_ADDRESS) {
      logger.warn(
        'SIPPY_SPENDER_ADDRESS not set in env. Using dynamically created wallet.'
      );
      ...
      logger.warn(
        'SIPPY_SPENDER_ADDRESS in env does not match created wallet!'
      );
